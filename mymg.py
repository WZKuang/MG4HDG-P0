...
# multigrid with variable smoothing
class MultiGrid2(BaseMatrix):
    def __init__ (self, mat, prol1, prol2, dofU, dofW, coarsedofs,  w1=0.9, sm="gs", var=False,
            nsmooth=1, wcycle=False, he=True, Mdofs=None):
        super(MultiGrid2, self).__init__()
        self.mats = [mat]
        self.smoothers = [ () ]
        self.w1 = w1
        self.sm = sm
        ...
        self.prol2 = prol2
        self.nlevels = 0
        self.dofsU = [dofU] # nothing
        self.dofsW = [dofW] # nothing
        self.he = he
        self.he_prol = [mat.Inverse(Mdofs, inverse="sparsecholesky")]
        self.invcoarseproblem = mat.Inverse(coarsedofs, inverse="sparsecholesky")

    def Update(self, mat, dofs, dofU, dofW, hp=None):
        self.mats.append (mat)
        if type(dofs)==list: # block smoother
            self.smoothers.append(mat.CreateBlockSmoother( dofs ) )
        else: # point smoother
            self.smoothers.append(mat.CreateSmoother( dofs ) )
        self.nlevels = len(self.mats)
        self.dofsU.append(dofU)
        self.dofsW.append(dofW)
        self.he_prol.append(hp)
    ...
    def MGM(self, level, b, rho):
        ...
        rho[:] = 0
        if self.var: # variable V cycle
            ms = self.nsmooth*2**(self.nlevels-level) # number of smoothers
        else:
            ms = self.nsmooth
        # set-zero V dofs in b??? TODO
        if level > 0:
            dofU0 = self.dofsU[level-1] # degrees of free for first component
            dofW0 = self.dofsW[level-1] # degrees of free for first component
            dofU = self.dofsU[level] # degrees of free for first component
            dofW = self.dofsW[level] # degrees of free for first component
            rhoprev = b.CreateVector()
            rhoprev0 = b.CreateVector()
            rhoprev[:] = 0 # set-zero 
            tmp = b.CreateVector()
            
            # hack for Eigenvalue Estimator?????
            b[dofU:] = 0 
            ...
            if self.sm =="gs": 
                ...
                for i in range(ms):
                    rho.data += self.w1*self.smoothers[level]*tmp # jacobi
                    tmp.data  = b - self.mats[level] * rho
            # harmonic extension (after smoothing)
            if self.he==True:
                 # he for rho
                 rho.data -= self.he_prol[level] @ self.mats[level] * rho
                 tmp.data = b - self.mats[level]*rho                    

            if self.wcycle>-1:
               self.prol1.Restrict(level, tmp[:dofU])
               # create temporary vectors
               tmp0, rho0 = CreateVVector(dofU0+dofW0), CreateVVector(dofU0+dofW0)
               tmp0[:] = 0
               tmp0[:dofU0].data = tmp[:dofU0]
               if self.he != True:
                    self.prol2.Restrict(level, tmp[dofU:]) 
                    tmp0[dofU0:].data = tmp[dofU:dofU+dofW0]
                    
               self.MGM(level-1, tmp0, rho0)
               rhoprev[:dofU0].data, rhoprev[dofU:dofU+dofW0].data= rho0[:dofU0], rho0[dofU0:]
               # W-cycle FIXME later
#                if level>1 and self.wcycle==1:
#                     # W cycle (FIXME)
#                     tmp[0:nc].data -= self.mats[level-1]*rhoprev[0:nc]
#                     self.MGM(level-1, tmp[0:nc] , rhoprev0[:nc])
#                     rhoprev[:nc].data += rhoprev0[:nc] #update
               self.prol1.Prolongate(level, rhoprev[:dofU])
               if self.he ==True: # harmonic extension prolongation
                  rhoprev.data -= self.he_prol[level] @ self.mats[level]*rhoprev
               else:
                  self.prol2.Prolongate(level, rhoprev[dofU:])
