...
#include "../utils/facetfespace0.hpp"
...
{ 
  ...
  FacetFESpace0 ::  FacetFESpace0 (shared_ptr<MeshAccess> ama, const Flags & flags, bool checkflags)
    : FESpace(ama, flags)
  {
    name="FacetFESpace0(facet)";
    type = "facet0";
    // defined flags
  ...
  FacetFESpace0 :: ~FacetFESpace0 ()
  { ; }


  FlatArray<VorB> FacetFESpace0 :: GetDualShapeNodes (VorB vb) const
  {
...
'projected jumps' HDG method.
  ...
  void FacetFESpace0 :: UpdateCouplingDofArray()
  {
    size_t ndof = GetNDof();
  ...
  FiniteElement & FacetFESpace0 :: T_GetFE (int elnr, Allocator & alloc) const
  {
  ...
  // ------------------------------------------------------------------------
  FiniteElement & FacetFESpace0 :: GetFE (ElementId ei, Allocator  & lh) const
  {
    if (!DefinedOn (ei))
      {
        return
  ...
  // ------------------------------------------------------------------------
  void FacetFESpace0 :: GetDofNrs (ElementId ei, Array<int> & dnums) const
  {
    dnums.SetSize0();
  ...
  // ------------------------------------------------------------------------
  shared_ptr<Table<int>> FacetFESpace0 :: CreateSmoothingBlocks (const Flags & precflags) const
  {
    if (all_dofs_together)
  ...
  shared_ptr<Array<int>> FacetFESpace0 :: CreateDirectSolverClusters (const Flags & precflags) const
  {
    if (all_dofs_together)
   ...
   void FacetFESpace0 :: SetOrder (NodeId ni, int order) 
  {
  ...
  int FacetFESpace0 :: GetOrder (NodeId ni) const
  {
    if (CoDimension(ni.GetType(), ma->GetDimension()) == 1)
      if (ni.GetNr() < order_facet.Size())
  ...
  static RegisterFESpace<FacetFESpace0> init_facet ("facet0");
}
